...
| `max_model_len` | 文本 64K；VLM 16K | 首发安全上限：文本不超过 65536，VLM 不超过 16384；后续压力验收通过后再放宽 | 不让 vLLM 继承可能超过单卡 KV Cache 容量的模型默认值。 |
...
| `max_model_len` | 下拉选择 16K/32K/64K | 文本默认 64K；VLM 默认 16K；后端即使调用方遗漏也会应用安全默认值。 | 不得超过模型和当前服务安全上限；过大可能导致显存不足。 |
